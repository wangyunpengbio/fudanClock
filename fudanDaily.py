...
class Fudan:
    ...
    def login(self):
        ...
        if post.status_code == 302:
            ...
            self.notify(_status=False, _message="◉登录失败，请检查账号信息", push_key=self.push_key)
            print("◉登录失败，请检查账号信息")
            self.close()
    ...
    # 使用爱语飞飞进行消息推送，http://iyuu.cn/
    def notify(self, _status, _message, push_key):
        if not push_key:
            return
        ...
        requests.post(f"https://iyuu.cn/{push_key}.send", data=_d, headers=headers, verify=False)
...
if __name__ == '__main__':
    ...
    daily_fudan = Zlapp(uid, psw, push_key,
